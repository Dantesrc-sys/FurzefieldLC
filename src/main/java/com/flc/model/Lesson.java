...
public class Lesson {
    ...
    public Lesson(String lessonId, ExerciseType exerciseType, Day day, TimeSlot timeSlot, int weekNumber) {
        if (lessonId     == null || lessonId.isBlank()) throw new IllegalArgumentException("Lesson ID cannot be empty");
        if (exerciseType == null)                       throw new IllegalArgumentException("ExerciseType cannot be null");
        ...
        if (weekNumber   < 1)                           throw new IllegalArgumentException("Week number must be >= 1");
    ...
    // ── Capacity ──────────────────────────────────────────────────────────────
    public int  getEnrolledCount() { return members.size(); }
    public int  getAvailableSpaces() { return AppConfig.MAX_LESSON_CAPACITY - members.size(); }
    public boolean isFull()  { return members.size() >= AppConfig.MAX_LESSON_CAPACITY; }
    public boolean isEmpty() { return members.isEmpty(); }

    // ── Enrolment ─────────────────────────────────────────────────────────────
    /**
     * Adds a member to this lesson.
     * @throws IllegalStateException    if the lesson is already full
     * @throws IllegalArgumentException if the member is already enrolled
     */
    public void addMember(Member member) {
        if (member == null)           throw new IllegalArgumentException("Member cannot be null");
        if (isFull())                 throw new IllegalStateException("Lesson is full");
        if (hasMember(member))        throw new IllegalArgumentException("Member already enrolled in this lesson");
        members.add(member);
    }

    /**
     * Removes a member from this lesson.
     * @throws IllegalArgumentException if the member is not enrolled
     */
    public void removeMember(Member member) {
        if (member == null)    throw new IllegalArgumentException("Member cannot be null");
        if (!hasMember(member)) throw new IllegalArgumentException("Member not enrolled in this lesson");
    ...
    public String toString() {
        return String.format("Lesson{id='%s', exercise='%s', week=%d, %s, %s, enrolled=%d/%d}",
                lessonId,
