...
public class ReportController {
    ...
    public List<AttendanceRow> getAttendanceReport() {
        ...
        for (Lesson lesson : store.getLessons()) {
            ...
            rows.add(new AttendanceRow(lesson.getWeekNumber(), lesson.getDay(), lesson.getTimeSlot(),
                    lesson.getExerciseType().getName(), lesson.getEnrolledCount(), avg));
        }

        // Sort: week → day → timeslot
        rows.sort(Comparator.comparingInt(AttendanceRow::weekNumber).thenComparing(AttendanceRow::day)
    ...
    public List<IncomeRow> getIncomeReport() {
        ...
        for (ExerciseType type : store.getExerciseTypes()) {
            ...
            rows.add(new IncomeRow(name, type.getPrice(), countMap.get(name), incomeMap.get(name)));
        }
    ...
    /**
     * Returns the single exercise type with the highest total income. Returns null if there are no lessons.
     */
     ...
     */
    public record AttendanceRow(int weekNumber, Day day, TimeSlot timeSlot, String exerciseName, int enrolledCount,
            double averageRating) {
        public String formattedRating() {
            return averageRating == 0.0 ? "No reviews" : String.format("%.1f / 5.0", averageRating);
        }
    }
     ...
     */
    public record IncomeRow(String exerciseName, double pricePerLesson, int totalEnrolled, double totalIncome) {
