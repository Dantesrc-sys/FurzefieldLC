...
public class TimetableScreen extends JPanel {
    ...
    private JPanel buildFilterBar() {
        ...
        dayCombo.addActionListener(e -> {
            ...
            if (filterMode.equals("DAY"))
                refresh();
        });

        // Exercise selector
        List<String> names = store.getExerciseTypes().stream().map(ExerciseType::getName).toList();
    ...
    private void refreshWithWeek(int weekFilter) {
        ...
        List<Lesson> lessons = filterMode.equals("DAY") ? bookingController.getLessonsByDay(selectedDay)
                : bookingController.getLessonsByExerciseName(selectedExercise);

        for (Lesson l : lessons) {
            if (weekFilter != 0 && l.getWeekNumber() != weekFilter)
                continue;

            int spaces = l.getAvailableSpaces();
            String status = l.isFull() ? "Full" : spaces == 1 ? "1 space left" : spaces + " spaces";

            tableModel.addRow(new Object[] { "Week " + l.getWeekNumber(), l.getDay().getDisplayName(),
                    l.getTimeSlot().getDisplayName(), l.getExerciseType().getName(),
                    "£" + String.format("%.2f", l.getPrice()), l.getEnrolledCount() + " / 4", spaces, status });
        }
    }
