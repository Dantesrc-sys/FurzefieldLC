...
public class ReportScreen extends JPanel {
    ...
    private JPanel buildHeader() {
        ...
        if (top != null)
            panel.add(buildHighlightBadge(top), BorderLayout.EAST);
        return panel;
    }
    ...
    private JPanel buildHighlightBadge(IncomeRow top) {
        ...
        badge.setBorder(
                BorderFactory.createEmptyBorder(Theme.SPACE_MD, Theme.SPACE_XL, Theme.SPACE_MD, Theme.SPACE_XL));
    ...
    private JPanel buildAttendanceReport() {
        ...
        panel.setOpaque(false);
        panel.add(reportTitle("Attendance and Rating", "Per lesson   sorted by week"), BorderLayout.NORTH);
        ...
        for (AttendanceRow row : rows) {
            model.addRow(new Object[] { "Wk " + row.weekNumber(), row.day().getDisplayName(),
                    row.timeSlot().getDisplayName(), row.exerciseName(), row.enrolledCount() + " / 4",
                    row.formattedRating() });
        }
        ...
        table.getColumnModel().getColumn(5).setCellRenderer(new ModernTable.DefaultRenderer() {
            @Override
            public Component getTableCellRendererComponent(JTable t, Object val, boolean sel, boolean foc, int row,
                ...
                if (s.equals("No reviews")) {
                    ...
                    try {
                        ...
                        setForeground(
                                avg >= 4.0 ? Theme.TEXT_SUCCESS : avg >= 3.0 ? Theme.TEXT_WARNING : Theme.TEXT_ERROR);
        ...
        panel.add(footerLabel("Total enrolled: " + totalEnrolled + " across all lessons"), BorderLayout.SOUTH);
        return panel;
    }
    ...
    private JPanel buildIncomeReport() {
        JPanel panel = new JPanel(new BorderLayout(0, Theme.SPACE_MD));
        panel.setOpaque(false);
        panel.add(reportTitle("Income by Exercise", "Highest earning first"), BorderLayout.NORTH);
        ...
        for (IncomeRow row : rows) {
            model.addRow(new Object[] { row.exerciseName(), "£" + String.format("%.2f", row.pricePerLesson()),
                    row.totalEnrolled(), row.formattedIncome() });
        }
        ...
        panel.add(ModernTable.wrap(table), BorderLayout.CENTER);
        panel.add(footerLabel("Grand total income: £" + String.format("%.2f", grandTotal)), BorderLayout.SOUTH);
        return panel;
    }
